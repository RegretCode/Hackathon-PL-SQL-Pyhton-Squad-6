...
def convert_join_to_pyspark(join_clauses, base_table) -> str:
    ...
    for join in join_clauses:
        ...
        # A estrutura real do JoinClause tem: join_type, table, condition
        # Vamos tentar usar a condição como está ou simplificar
        join_table = join.table
        
        # Tentar extrair condição de join simples do formato "table1.col = table2.col"
        condition_match = re.search(r'(\w+)\.(\w+)\s*=\s*(\w+)\.(\w+)', join.condition)
        if condition_match:
            left_table, left_col, right_table, right_col = condition_match.groups()
            join_condition = f"F.col('{left_col}') == {join_table}_df['{right_col}']"
        else:
            # Fallback: usar a condição como string (pode precisar de ajustes)
            join_condition = f"'{join.condition}'"
        
        # Adicionar o join
        join_code += f".{join_method}({join_table}_df, {join_condition}{join_how})"
